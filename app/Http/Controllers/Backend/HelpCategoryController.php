...
class HelpCategoryController extends Controller
{
    ...
    public function getdetails(Request $request)
    {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('active_icon', function($data){
                    $img = '<img src="'.uploaded_asset($data->active_icon).'" style="width: 50%">';
                ...
                ->rawColumns(['action','status','icon','active_icon'])
                ->make(true);
    ...
    {        
        ...
        if( $data == null ){
            ...
            if($request->icon == null){
                ...
                if($request->active_icon == null){
                    return back()->withErrors('Please Select an Active Icon');
                }else{           

                    $add = new HelpCategory;

                    $add->icon = $request->icon;
                    $add->active_icon = $request->active_icon;                
    ...
    {        
        ...
        if($request->icon == null){
            ...
            if($request->active_icon == null){
                ...
                $update->active_icon = $request->active_icon;                
